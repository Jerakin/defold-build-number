...
return function()
	...
	describe("Default options gives the expected version string.", function()
		...
		test("double prerelease tag no version", function()
			assert(version.version("1.4.10-alpha.beta", "1", "gfb6eb15", true):format(opts) == "1.4.10-alpha.beta.1.dev1+ggfb6eb15.d" .. os.date("%Y%m%d"))
		end)
		test("double prerelease tag with version", function()
			assert(version.version("1.4.10-alpha.beta.1", "1", "gfb6eb15", true):format(opts) == "1.4.10-alpha.beta.2.dev1+ggfb6eb15.d" .. os.date("%Y%m%d"))
		end)
	end)
end
